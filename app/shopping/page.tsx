        ...
        {items.map((item) => (
          <div key={item.id} className="card space-y-2">
            <div>
              ...
              <p className="text-sm text-workshop-700">{[item.storageLocation, item.bin].filter(Boolean).join(" / ")}</p>
            </div>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-workshop-100">
                ...
                <td className="px-2 py-2">{[item.storageLocation, item.bin].filter(Boolean).join(" / ")}</td>
