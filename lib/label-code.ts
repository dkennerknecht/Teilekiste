...
export async function assignNextLabelCode(areaId: string, typeId: string) {
  return prisma.$transaction(async (tx) => {
    const [area, type, config] = await Promise.all([
      tx.area.findUniqueOrThrow({ where: { id: areaId } }),
      tx.labelType.findUniqueOrThrow({ where: { id: typeId } }),
      tx.labelConfig.upsert({ where: { id: "default" }, update: {}, create: { id: "default" } })
    ]);
